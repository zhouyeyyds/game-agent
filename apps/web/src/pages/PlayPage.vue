<template>
  <main class="mx-auto max-w-[1800px] px-6 py-7 lg:px-12">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <div class="flex flex-wrap items-center gap-3">
            <h1 class="m-0 text-3xl font-black text-slate-950">{{ descriptor?.title || manifest?.title || '远程游戏' }}</h1>
          ...
          <p class="mt-2 text-sm text-slate-500">{{ statusLine }}</p>
        </div>
      </div>
    </div>

    <template v-if="loading">
      <div class="grid gap-6 xl:grid-cols-[1fr_480px]">
        <section class="agent-card flex min-h-[680px] items-center justify-center overflow-hidden p-8">
          <div class="relative w-full overflow-hidden rounded-2xl bg-slate-950 p-10 text-center text-white">
            ...
            <div class="relative z-10 mx-auto max-w-xl py-24">
              ...
              <h2 class="m-0 text-4xl font-black">正在加载游戏...</h2>
              <p class="mt-5 text-white/70">首次加载可能需要较长时间，请稍候，我们正在为你准备最佳体验。</p>
    ...
    <template v-else-if="error">
      <div class="grid gap-6 xl:grid-cols-[1fr_480px]">
        <section class="agent-card overflow-hidden p-8">
          <div class="grid min-h-[520px] gap-8 lg:grid-cols-[0.8fr_1fr]">
            <div class="relative overflow-hidden rounded-3xl bg-indigo-50">
            ...
            <div class="flex flex-col justify-center">
              ...
              <h2 class="m-0 text-4xl font-black text-slate-950">游戏加载失败</h2>
      ...
      <div class="grid gap-6 xl:grid-cols-[1fr_420px]">
        <section class="space-y-5">
          ...
          <section class="agent-card p-5">
            <h2 class="m-0 mb-4 text-lg font-black text-slate-950">相关游戏</h2>
            <div class="grid gap-3 md:grid-cols-4">
              <div v-for="title in relatedGames" :key="title" class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3">
                <img :src="referenceImages.playRunningImage" alt="" class="h-16 w-24 rounded-xl object-cover object-left-top" />
                <div>
                  <p class="m-0 text-sm font-black text-slate-900">{{ title }}</p>
...
const statusLine = computed(() => {
  if (loading.value) return '加载资源中，请稍候'
...
const GameInfoSide = defineComponent({
  ...
  setup(sideProps) {
    return () => h('section', { class: 'agent-card p-5' }, [
      h('img', { src: sideProps.failed ? referenceImages.playErrorImage : referenceImages.playRunningImage, class: 'h-44 w-full rounded-2xl object-cover object-left-top', alt: '' }),
      h('h2', { class: 'mt-5 text-2xl font-black text-slate-950' }, sideProps.manifest?.title || sideProps.descriptor?.title || '迷雾之城：哀歌'),
      h('p', { class: 'mt-3 text-sm leading-7 text-slate-600' }, '在迷雾与诅咒交织的城市中探寻真相，你的选择将决定众人的命运。'),
      h('div', { class: 'mt-4 flex flex-wrap gap-2' }, ['冒险', '解谜', '角色扮演', '剧情'].map((tag) => h('span', { class: 'rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-600' }, tag))),
      h('div', { class: 'mt-5 grid grid-cols-2 gap-3 rounded-2xl border border-slate-200 p-4 text-center' }, [
        h('div', [h('p', { class: 'm-0 text-xl font-black text-slate-950' }, '12.4k'), h('p', { class: 'm-0 text-xs text-slate-500' }, '游玩次数')]),
        h('div', [h('p', { class: 'm-0 text-xl font-black text-slate-950' }, '92%'), h('p', { class: 'm-0 text-xs text-slate-500' }, '好评率')]),
      ]),
    ])
  },
})
