<template>
  <article class="agent-card group overflow-hidden transition duration-200 hover:-translate-y-1 hover:shadow-xl">
    <button class="block w-full border-0 bg-transparent p-0 text-left" type="button" @click="emit('play', game.id)">
      ...
      <div class="space-y-3 p-4">
        <h3 class="m-0 line-clamp-1 text-lg font-black text-slate-950">{{ game.title }}</h3>
        <p class="m-0 line-clamp-2 min-h-10 text-sm leading-5 text-slate-600">{{ game.description }}</p>
        <div class="flex items-center gap-2 text-sm text-slate-500">
          <span class="grid h-6 w-6 place-items-center rounded-full bg-slate-900 text-xs font-bold text-white">{{ game.author.displayName.slice(0, 1) }}</span>
